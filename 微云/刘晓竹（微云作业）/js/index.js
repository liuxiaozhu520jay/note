...
//		folders.appendChild(newfileItem);
	...
	var fileMovePathTo = document.querySelector(".fileMovePathTo")//提示字
	var notRemoveArr = [];
	//点击移动
	...
	confirm.onclick = function(){
		...
		if(toElement){//如果有这个元素
			...
			Array.from(selectedArr).forEach(function(item){
				...
				data[item].pid = toId;//将要移动的这个文件的父ID设置为目标文件夹的的  ID
			});
		...
		Array.from(newTree).forEach(function(item){
			item.onclick=function(){
				...
				if(notremove=="1"){
					//不可以移动
//					alert("不可以移动")
					fileMovePathTo.innerHTML = "不可以移动";
				}else{
					movePositionElement(this.dataset.id);//菜单点击定位，变颜色
					fileMovePathTo.innerHTML = " ";
				}
			}
		});
	}
